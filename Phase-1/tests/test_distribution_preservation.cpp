 ...
 */
TEST_CASE("Test1.8_DistributionPreservation", "[distribution]") {
    int n = 1024;
    int d = 64;
    uint32_t d_k = d;
    ...
    REQUIRE(ks_stat < 0.01f);
}

/**
 * Test histogram similarity
 */
TEST_CASE("Test_Distribution_HistogramBins", "[distribution]") {
    int n = 256;
    int d = 32;
    uint32_t d_k = d;
    ...
    REQUIRE(populated_bins >= 3);  // At least some spread
}
